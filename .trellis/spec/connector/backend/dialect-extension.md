...
public abstract class AbstractJdbcDialect implements Dialect {
    ...
    // 可选钩子: 表/列注释补查(默认空 map,用 DatabaseMetaData 的 REMARKS)
    protected Map<String, String> resolveTableComments(Connection conn, String schema) throws SQLException { return Collections.emptyMap(); }
    protected Map<String, String> resolveColumnComments(Connection conn, String schema, String table) throws SQLException { return Collections.emptyMap(); }
}
```

**基类已实现**:
- `connect`: driver.connect 通用加载(支持外置 jar 的 URLClassLoader 隔离)
- `listTables`: `DatabaseMetaData.getTables` 标准遍历,注释来自 `resolveTableComments` 或 REMARKS
- `getColumns`: `getColumns` + `getPrimaryKeys` 标准遍历,REMARKS 空时取 `resolveColumnComments`
- `getIndexes`: `getIndexInfo` 标准遍历,跳过主键索引

### Common Mistake: Oracle 注释丢失

**Symptom**: Oracle 导入的表/字段中文注释为空(用了表名/列名兜底)。
**Cause**: Oracle JDBC `DatabaseMetaData.getTables/getColumns` 的 `REMARKS` 常返回空,
注释实际存在数据字典 `USER_TAB_COMMENTS`/`USER_COL_COMMENTS`。

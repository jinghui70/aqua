...
## 3. 反解(物理类型 -> 逻辑类型)分库归属
- **Java 库**(Oracle/信创/H2):反解在 Java 侧,**外置 resolver jar**(URLClassLoader + registry.json,与 JDBC 驱动同一套加载机制)。加库放 resolver jar,connector 本体零改动。
...
- **实现方式**:
  ...
  - DDL 生成规则可外置配置
- **扩展方式**:
  1. 在 connector.jar 添加新的 Dialect 实现类
  2. 在 registry.json 注册
  3. 提供 JDBC 驱动 jar(用户自备)
...
- JDBC 驱动 + resolver jar:用户提供(`drivers/` + `registry.json`)。
...
aqua/
  ...
  drivers/                # JDBC 驱动 jar + resolver jar(用户提供)
  registry.json           # 数据库注册表(urlPattern/driverClass/resolverClass)
...
- Q1 一次性命令 + 反解移 Java
- Q2 反解 Java 侧外置 jar(URLClassLoader + registry.json)
- Q3 混合架构 + 反解分库归属
